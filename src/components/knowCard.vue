<template>
  <div class="col">
    <big-card :card-datas="datas">
      <!--slot {{{1-->
      <q-card-section>
        <div class="row items-center text-center">
          <q-btn
            class="col q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://www.vuejs.org"
          >
            <!-- vue {{{2 -->
            <svg width="40%" viewBox="0 0 261.76 226.69">
              <g transform="matrix(1.3333 0 0 -1.3333 -76.311 313.34)">
                <g transform="translate(178.06 235.01)">
                  <path
                    d="m0 0-22.669-39.264-22.669 39.264h-75.491l98.16-170.02 98.16 170.02z"
                    fill="#41b883"
                  />
                </g>
                <g transform="translate(178.06 235.01)">
                  <path
                    d="m0 0-22.669-39.264-22.669 39.264h-36.227l58.896-102.01 58.896 102.01z"
                    fill="#34495e"
                  />
                </g>
              </g>
            </svg>
            <!-- }}} -->
          </q-btn>
          <q-btn
            class="col q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://www.quasar.dev"
          >
            <!-- Quasar {{{2 -->
            <svg width="40%" viewBox="0 0 126 126">
              <g id="layer1" transform="translate(-118.66081,-453.70793)">
                <g id="g5519-9-0" transform="translate(2201.4292,498.40539)">
                  <g
                    id="g4636-5-7-7"
                    transform="translate(14.241949,-1.4302574e-5)"
                  >
                    <g id="g4768-9-1-2" transform="translate(-29.798982,0)">
                      <g
                        id="g5443-0-1-3-9-3"
                        transform="matrix(0.55595317,0,0,0.55595317,-1486.6013,-286.79579)"
                      >
                        <g
                          id="g8856-6-1-1-9-9-3-3"
                          transform="matrix(0.09527033,0,0,0.09527033,-1695.2716,706.62921)"
                        >
                          <circle
                            id="circle8858-1-3-7-6-9-7-2"
                            style="fill:#FFFFFF;"
                            cx="8020.4"
                            cy="-1658.3"
                            r="1188"
                          />
                          <path
                            id="path8860-5-4-8-2-7-1-8"
                            style="fill:#263238;"
                            d="M8214.8-1658.3c0,107.3-87,194.4-194.4,194.4h0
							c-107.3,0-194.4-87-194.4-194.4c0,0,0,0,0,0c0-107.3,87-194.4,194.4-194.4S8214.8-1765.6,8214.8-1658.3z"
                          />
                          <path
                            id="path8862-5-5-9-1-9-2-7"
                            style="fill:#1976D2;"
                            d="M8883.2-2156.5c-42.3-72.7-93.6-139.8-152.7-199.6l-224.7,129.7
							c-70-59.8-149.6-105.6-234.6-136c-76.7,77.7-137.6,164.6-181.6,259.1c250.1-17,508.4,73.5,748.2,260.9l141.4-81.6
							C8957.2-2005.3,8924.9-2083.4,8883.2-2156.5L8883.2-2156.5z"
                          />
                          <path
                            id="path8864-4-8-1-2-7-3-1"
                            style="fill:#42A5F5;"
                            d="M8883.3-1160.2c41.8-73,74.3-150.9,96.5-232L8755.1-1522
							c16.8-90.6,16.7-182.4,0.5-271.1c-105.7-27.6-211.4-36.9-315.2-27.7c139.8,208.1,190.6,477.1,148.2,778.4l141.4,81.6
							C8789.3-1020.5,8840.8-1087.5,8883.3-1160.2L8883.3-1160.2z"
                          />
                          <path
                            id="path8866-7-5-5-0-3-5-6"
                            style="fill:#1976D2;"
                            d="M8020.5-662c84.1-0.2,167.8-11.1,249.2-32.4v-259.5
							c86.8-30.7,166.3-76.8,235-135.2c-29-105.3-73.7-201.5-133.6-286.8c-110.3,225.1-317.9,403.6-600,517.6V-695
							C7852.5-673.5,7936.3-662.5,8020.5-662z"
                          />
                          <path
                            id="path8868-6-7-4-7-5-7-1"
                            style="fill:#42A5F5;"
                            d="M7157.7-1160.1c42.3,72.7,93.6,139.8,152.7,199.6l224.7-129.7
							c70,59.8,149.6,105.6,234.6,136c76.7-77.7,137.6-164.6,181.6-259.1c-250.1,17-508.4-73.5-748.2-260.9l-141.4,81.6
							C7083.7-1311.3,7115.9-1233.2,7157.7-1160.1L7157.7-1160.1z"
                          />
                          <path
                            id="path8870-5-3-9-3-0-0-9"
                            style="fill:#1976D2;"
                            d="M7157.6-2156.4c-41.8,73-74.3,150.9-96.5,232l224.7,129.7
							c-16.8,90.6-16.7,182.4-0.5,271.1c105.7,27.6,211.4,36.9,315.2,27.7c-139.8-208.1-190.6-477.1-148.2-778.4l-141.4-81.6
							C7251.6-2296.1,7200.1-2229.1,7157.6-2156.4z"
                          />
                          <path
                            id="path8872-6-3-2-1-4-2-1"
                            style="fill:#42A5F5;"
                            d="M8020.4-2654.6c-84.1,0.2-167.8,11.1-249.2,32.4v259.4
							c-86.8,30.7-166.3,76.8-235,135.2c29,105.3,73.7,201.5,133.6,286.8c110.3-225.1,317.9-403.6,600-517.6v-163.3
							C8188.4-2643.1,8104.6-2654.1,8020.4-2654.6L8020.4-2654.6z"
                          />
                        </g>
                      </g>
                    </g>
                  </g>
                </g>
              </g>
            </svg>
            <!-- }}} -->
          </q-btn>
          <q-btn
            class="col q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://firebase.google.com"
          >
            <!-- Firebase{{{2 -->
            <svg width="40%" viewBox="23 6 132 132">
              <defs>
                <linearGradient
                  id="a"
                  x1="-3.49%"
                  x2="100.83%"
                  y1="17.02%"
                  y2="92.9%"
                >
                  <stop offset="0%" stop-col-4or="#fff" stop-opacity=".1" />
                  <stop offset="14%" stop-col-4or="#fff" stop-opacity=".08" />
                  <stop offset="61%" stop-col-4or="#fff" stop-opacity=".02" />
                  <stop offset="100%" stop-col-4or="#fff" stop-opacity="0" />
                </linearGradient>
                <path
                  id="b"
                  d="M106.687 35.2742c-.186-1.0977-.967-2-2.0244-2.338s-2.2148-.057-3.0002.73L86.2473 49.166l-12.12-23.1455c-.5133-.9786-1.525-1.5914-2.6273-1.5914s-2.114.6128-2.6273 1.5914l-6.6277 12.656L45.62 7.5726c-.603-1.1297-1.8588-1.746-3.118-1.5297s-2.2394 1.216-2.4335 2.4827L24 111.701l42.9727 24.1654c2.6985 1.5113 5.985 1.5113 8.6836 0L119 111.701l-12.313-76.427z"
                />
              </defs>
              <path
                fill="#ffa000"
                d="M23.8333 111.7187L39.9655 8.4906c.1972-1.266 1.1818-2.264 2.445-2.4786s2.522.4028 3.126 1.5327L62.22 38.662l6.65-12.6615c.515-.979 1.5303-1.592 2.6366-1.592s2.1215.613 2.6366 1.592l45.0227 85.718H23.8333z"
              />
              <path
                fill="#f57c00"
                d="M79.566 71.5074L62.2124 38.6472l-38.379 73.0715L79.566 71.5074z"
              />
              <path
                fill="#ffca28"
                d="M119.1666 111.7187l-12.356-76.4603c-.1867-1.098-.9703-2-2.0315-2.34s-2.2226-.057-3.0107.7302l-77.935 78.069 43.1234 24.1834c2.708 1.512 6.006 1.512 8.714 0l43.4958-24.1834z"
              />
              <path
                fill="#fff"
                fill-opacity=".2"
                d="M106.8105 35.2584c-.1867-1.098-.9703-2-2.0315-2.34s-2.2226-.057-3.0107.7302L86.3 49.1562 74.1365 26c-.515-.979-1.5303-1.592-2.6366-1.592s-2.1215.613-2.6366 1.592l-6.65 12.6615L45.529 7.5447c-.605-1.1302-1.8653-1.7466-3.13-1.5304S40.153 7.231 39.958 8.498L23.8333 111.7187h-.052l.052.0596.4245.2085 77.488-77.5775c.7877-.7915 1.952-1.076 3.016-.737s1.849 1.2445 2.0338 2.3457l12.2518 75.775.1192-.0745-12.356-76.4603zm-82.8357 76.3188L39.9655 9.228c.1948-1.267 1.1784-2.2675 2.442-2.4837s2.524.4 3.13 1.5304L62.22 39.392l6.65-12.6615c.515-.979 1.5303-1.592 2.6366-1.592s2.1215.613 2.6366 1.592l11.9167 22.664-62.0858 62.1827z"
              />
              <path
                fill="#a52714"
                opacity=".2"
                d="M75.6708 135.1722c-2.708 1.512-6.006 1.512-8.714 0L23.9376 111.056l-.1043.663 43.1234 24.176c2.708 1.512 6.006 1.512 8.714 0l43.4958-24.176-.1117-.6852-43.384 24.1387z"
              />
              <use fill="url(#a)" xlink:href="#b" />
            </svg>
            <!-- }}} -->
          </q-btn>
        </div>
        <div class="row text-center text-h5 text-weight-light">
          <div class="col">VueJS</div>
          <div class="col">Quasar</div>
          <div class="col">Firebase</div>
        </div>
        <div class="row items-center q-mt-sm">
          <q-btn
            class="col-4 text-center q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://www.typescriptlang.org"
          >
            <!-- Typescript{{{2 -->
            <svg width="40%" viewBox="0 0 400 400">
              <path d="M0 200V0h400v400H0" fill="#007acc" />
              <path
                d="M87.7 200.7V217h52v148h36.9V217h52v-16c0-9 0-16.3-.4-16.5 0-.3-31.7-.4-70.2-.4l-70 .3v16.4l-.3-.1zM321.4 184c10.2 2.4 18 7 25 14.3 3.7 4 9.2 11 9.6 12.8 0 .6-17.3 12.3-27.8 18.8-.4.3-2-1.4-3.6-4-5.2-7.4-10.5-10.6-18.8-11.2-12-.8-20 5.5-20 16 0 3.2.6 5 1.8 7.6 2.7 5.5 7.7 8.8 23.2 15.6 28.6 12.3 41 20.4 48.5 32 8.5 13 10.4 33.4 4.7 48.7-6.4 16.7-22 28-44.3 31.7-7 1.2-23 1-30.5-.3-16-3-31.3-11-40.7-21.3-3.7-4-10.8-14.7-10.4-15.4l3.8-2.4 15-8.7 11.3-6.6 2.6 3.5c3.3 5.2 10.7 12.2 15 14.6 13 6.7 30.4 5.8 39-2 3.7-3.4 5.3-7 5.3-12 0-4.6-.7-6.7-3-10.2-3.2-4.4-9.6-8-27.6-16-20.7-8.8-29.5-14.4-37.7-23-4.7-5.2-9-13.3-11-20-1.5-5.8-2-20-.6-25.7 4.3-20 19.4-34 41-38 7-1.4 23.5-.8 30.4 1l-.2.2z"
                fill="#fff"
              />
            </svg>
            <!-- }}} -->
          </q-btn>
          <q-btn
            class="col-4 text-center q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://www.vuetifyjs.com"
          >
            <!-- vuetify{{{2 -->
            <svg width="35%" viewBox="0 0 2187.5 2500">
              <path d="M1093.8 0h-511l511 1208" fill="#1697f6" />
              <path
                d="M1093.8 1562.5V2500L0 364.5h573zm0-1562.5h511l-511 1208"
                fill="#7bc6ff"
              />
              <path d="M1614.5 364.5h573L1093.8 2500v-937.5z" fill="#aeddff" />
            </svg>

            <!-- }}} -->
          </q-btn>
          <q-btn
            class="col-4 text-center q-px-lg bouton"
            fab
            flat
            type="a"
            target="_blank"
            href="https://www.figma.com"
          >
            <!-- figma{{{2 -->
            <svg viewBox="0 0 200 300" width="28%">
              <path
                d="M50 300c27.6 0 50-22.4 50-50v-50H50c-27.6 0-50 22.4-50 50s22.4 50 50 50z"
                fill="#0acf83"
              />
              <path
                d="M0 150c0-27.6 22.4-50 50-50h50v100H50c-27.6 0-50-22.4-50-50z"
                fill="#a259ff"
              />
              <path
                d="M0 50C0 22.4 22.4 0 50 0h50v100H50C22.4 100 0 77.6 0 50z"
                fill="#f24e1e"
              />
              <path
                d="M100 0h50c27.6 0 50 22.4 50 50s-22.4 50-50 50h-50V0z"
                fill="#ff7262"
              />
              <path
                d="M200 150c0 27.6-22.4 50-50 50s-50-22.4-50-50 22.4-50 50-50 50 22.4 50 50z"
                fill="#1abcfe"
              />
            </svg>
            <!-- }}} -->
          </q-btn>
        </div>
        <div class="row text-center text-h5 text-weight-light">
          <div class="col">Typescript</div>
          <div class="col">Vuetify</div>
          <div class="col">Figma</div>
        </div>
      </q-card-section>
      <!-- }}} -->
    </big-card>

    <small-card :card-datas="datas">
      <!--slot{{{1-->
      <!--}}}-->
    </small-card>
  </div>
</template>

<script lang="ts">
//Imports{{{1
import { Vue, Component } from 'vue-property-decorator';
import { CardDatas } from 'components/models/models';
import smallCard from 'components/modules/smallCard.vue';
import bigCard from 'components/modules/bigCard.vue';
//}}}

@Component({
  //declare components{{{1
  components: { smallCard, bigCard }
  //}}}
})
export default class Card extends Vue {
  //datas{{{1
  datas: CardDatas = {
    cardPicture: 'statics/img/libraryKnow.jpg',
    cardTitle: 'Know',
    cardSubtitle: 'Advanced tools',
    subCardTitle: 'Use modern and advanced tools',
    subCardConclusion:
      'To respect <span class="text-italic">your</span> schedule',
    topRight: false,
    bottomLeft: false
  };
  //}}}
}
</script>

<style scoped>
#bigSVG {
  max-width: 480px;
}
#smallSVG {
  max-width: 200px;
}
#vue {
  width: 34%;
}
#quasar {
  background-color: red;
}
.bouton {
  height: 100px;
}
</style>
